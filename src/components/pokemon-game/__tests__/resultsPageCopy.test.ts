...
describe("copyTextToClipboard", () => {
	it("returns success when clipboard write succeeds", async () => {
		...
		await expect(
			copyTextToClipboard("https://example.com", writeText, fallbackCopy),
...
describe("shareOrCopyUrl", () => {
	it("uses native share when available", async () => {
		...
		await expect(
			shareOrCopyUrl("text", "url", true, share, writeClipboard),
	...
	it("copies to clipboard when native share is unavailable", async () => {
		...
		await expect(
			shareOrCopyUrl("text", "url", false, share, writeClipboard),
	...
	it("falls back to clipboard when share fails", async () => {
		...
		await expect(
			shareOrCopyUrl("text", "url", true, share, writeClipboard),
		).resolves.toBe("clipboardFallback");
	});
