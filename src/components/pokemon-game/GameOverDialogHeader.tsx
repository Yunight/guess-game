...
import { GameOverCompleteTitle } from "./GameOverCompleteTitle";
...
export const GameOverDialogHeader = ({
	...
	playerName,
	selectedGeneration,
}: GameOverDialogHeaderProps): ReactNode => {
		...
		<DialogHeader className="space-y-4">
			<div className="flex justify-center -mt-5">
				<div className="bg-white p-4 rounded-full shadow-xl relative overflow-visible">
					{isComplete && <GameOverFireworks generationName={selectedGeneration.name} />}
					<div className={`relative h-12 w-12 ${isComplete ? "animate-bounce" : "animate-pulse"}`}>
			...
			<DialogTitle className="text-2xl font-bold text-center">
				{isComplete ? <GameOverCompleteTitle language={i18n.language} /> : t("gameOver")}
			</DialogTitle>
			<DialogDescription asChild>
				<div className="text-center text-gray-200">
					{isComplete ? (
						<div className="space-y-2">
							<p className="text-yellow-100">{t("congratulations", { name: playerName })}</p>
