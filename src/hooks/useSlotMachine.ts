...
interface UseSlotMachineResult {
	...
	runSlotMachineEffect: (finalPokemonId: number, onComplete: (pokemonId: number) => void) => void;
	resetSlotMachine: () => void;
}

export const useSlotMachine = (selectedGeneration: Generation): UseSlotMachineResult => {
	const [isSlotMachineRunning, setIsSlotMachineRunning] = useState(false);
	const [spinningPokemonId, setSpinningPokemonId] = useState<number | null>(null);
	...
	const runSlotMachineEffect = useCallback(
		(finalPokemonId: number, onComplete: (pokemonId: number) => void): void => {
			...
			const showNextPokemon = (): void => {
				if (shouldContinueSpinning(slotMachineCountRef.current, SLOT_MACHINE_MIN_SPINS)) {
					const displayPokemonId = pickSpinDisplayId(rewards, lastDisplayedId, finalPokemonId);
